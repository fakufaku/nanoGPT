...
This is actually identical to enwik8 but uses a fixed vocab size of 256.
Instead of restricting to the 205 unique bytes found in the dataset.
"""
import io
import os
import pickle
...
print(f"{max(chars)=}, {min(chars)=}")
vocab_size = 256
# print("all the unique characters:", "".join(chars))
print(f"vocab size: {vocab_size:,}")

# create a mapping from bytes to bytes
# this is just a hack to re-use the existing
# code
stoi = list(range(256))
itos = stoi
